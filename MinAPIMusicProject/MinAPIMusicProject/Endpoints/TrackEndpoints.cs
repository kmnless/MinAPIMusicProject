...
public static class TrackEndpoints
{
    public static void AddTrackEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        app.MapPost("/{trackId}/like", async (MusicContext context, [FromRoute]int trackId, [FromQuery]int userId) =>
        {
            ...
            if (user.LikedTracksIds.Contains(trackId))
            {
                return Results.BadRequest("Track already liked.");
            }

            user.LikedTracksIds.Add(trackId);
            track.LikesCount++;
